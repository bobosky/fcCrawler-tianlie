...
import com.zj.parkExec.Company51JobExecThread;
 ...
 *
 */
public class MainCompany51JobThread implements Runnable{
	private static Logger log = Logger.getLogger(MainCompany51JobThread.class);
	private QueueBean queueBean=null;
	private QueueBean queueFirstBean=null;
	private String time=DateFormat.parse(new Date());
	public MainCompany51JobThread(QueueBean queueBean,QueueBean queueFirstBean)
	{
	...
	{
		//初始化静态文件
		MainStatic.setCompanyCodeNull();
		//入库队列信息
		IntoDb intoDb=new IntoDb(queueBean,queueBean.getQueueOutputDatabase(0),queueBean.getQueueOutputName(0),1,6,null,queueBean.getOutQueueGetTime());
		Thread intoDbl=null;
		...
		//初始化队列信息
		//队列是否初始化
		Company51JobAntQueue queue=new Company51JobAntQueue();
		queue.init(queueBean,queueBean.getQueueInputName(0),queueBean.isInputQueueInit(),intoDb);

		
		Company51JobExecThread execThread=new Company51JobExecThread(queueBean,queueFirstBean,queue,intoDb);
