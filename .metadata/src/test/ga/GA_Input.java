...
public class GA_Input implements Runnable {
	...
	public GA_Input(String[] x, double[][] rangeValue, int precision,
			...
			int matingCount, String matingContext, double mutationP,boolean isUpMax) {
...
//			}
	...
	public void run() {
		...
		//		System.out.println("执行外部方法");
			...
			if(addThreadStepCount()==tempCount)
			{
				...
				threadStepCount=0;
				//激活其他线程继续执行
				//System.out.println("激活其他所有线程--------------"+Thread.currentThread().getName());
				synchronized (this) {
					tempCount=1;
				this.notifyAll();
				try {
	...
	public static void main(String[] args) {
		...
		GA_Input ga_input = new GA_Input(x, rangeValue, precision, yCount, fx, false,
				px, count, context, py,false);
