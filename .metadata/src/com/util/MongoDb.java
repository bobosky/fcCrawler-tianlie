...
public class MongoDb {
	 ...
	 */
		...
		public MongoDb(String ipPort,String database)
		{
            //创建连接
			try {
				m= new Mongo(ipPort.substring(0,ipPort.indexOf(":")),Integer.parseInt(ipPort.substring(ipPort.indexOf(":")+1)));
	            //得到数据库
		        db=m.getDB(database);
			} catch (UnknownHostException e) {
				e.printStackTrace();
...
//         
		 ...
		 * @param doc
		 * @param doc2
		 * @param upsert 如果 为true则表示不存在就添加 否则 只更新不添加
		 * @param multi 如果为true 则表示 值更新查找到的第一个 否则 为全部更新
		 */
