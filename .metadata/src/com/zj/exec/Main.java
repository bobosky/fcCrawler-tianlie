...
public class Main {
	 ...
	 * 存储有效的Runnable
	 */
	private static List<Runnable> taskList = null;
	/**
	 * 存储有效的队列信息
	 */
	private static List<QueueBean> taskBean = null;

	/**
	 * 存储执行的线程
	 */
	private static List<Thread> taskListThread = null;
	...
	public static void run() {
		...
		for (int i = 0; i < taskQueueBeanList.size(); i++) {
			if (taskQueueBeanList.get(i).isRun()) {
				...
				{
					// 如果为执行
					Runnable run1 = run(taskQueueBeanList.get(i));
					taskList.add(run1);
	 ...
	 * @param name
	 */
	public static Runnable run(QueueBean bean) {
		if (bean.getName().equals("parking")) {
			return new MainParkingThread(bean);
		} else if (bean.getName().equals("jobs")) {
			System.out.println("job:" + bean.isExecIsRun());
			return new MainJobsThread(bean);
		} else if (bean.getName().equals("subwayAndBus")) {
			return new MainSubwayAndBusThread(bean);
		} else if (bean.getName().equals("busAndStation")) {
			return new MainBusAndStationThread(bean);
		} else if (bean.getName().equals("fang")) {
			return new MainFangThread(bean);
		} else if (bean.getName().equals("company")) {
			return new MainCompany51JobThread(bean);
		} else if (bean.getName().equals("companyEnd")) {
			return new MainCompany51JobGetComThread(bean);
		} else if (bean.getName().equals("companySearch")) {
			return new MainCompanyZhaopinThread(bean);
		} else if (bean.getName().equals("companySearchEnd")) {
			return new MainCompanyZhaopinGetComThread(bean);
		} else if (bean.getName().equals("companyGetJobQueue")) {
			return new MainCompany51JobDescThread(bean);
		} else if (bean.getName().equals("companyGetJobDesc")) {
			return new MainCompany51JobDescJobThread(bean);
		} else if(bean.getName().equals("dianpingArea"))
		{
			return new MainBussinessAreaThread(bean);
		}
		return null;
	}
	...
	public static void main(String[] args) {
		...
		if (args.length == 0 || args[0].equals("help")) {
			...
			System.out.println(" 执行 搜房 mongo数据更新");
			System.out.println("java -jar xxx.jar intoMongUpdate fangUpdate|(or file path) ip port database collection printcount");
			System.out.println("java -jar xxx.jar intoMongUpdate fangUpdate|(or file path) collectionName");
		...
		} else if (args[0].equals("intoMongUpdate")) {
			// 如果为爬虫则
			String file = "";
