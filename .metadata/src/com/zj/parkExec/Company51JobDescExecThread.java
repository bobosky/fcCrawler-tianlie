...
public class Company51JobDescExecThread implements Runnable{
	 ...
	 * 初始化程序
	 */
	public Company51JobDescExecThread(QueueBean queueBean,Company51JobDescAntQueue queue,IntoDb intoDb,IntoDb intoDb2)
	{
		this.queue=queue;
		this.intoDb=intoDb;
	...
	public void run()
	{
		...
		while(true)
		{
			...
			Company51JobBean bean=queue.get(queueBean);
			if(bean==null)
			{
				continue;
			}else if (!bean.getCompanyUrl().contains("search.51job.com"))
			{
